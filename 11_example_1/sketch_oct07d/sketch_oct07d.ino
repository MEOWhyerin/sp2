...
void loop() {
  ...
  } else if(_TARGET_LOW <= dist_ema && dist_ema <= _TARGET_HIGH){
    // myservo.writeMicroseconds(map(dist_ema,180,360,_DUTY_MIN,_DUTY_NEU));
    myservo.writeMicroseconds((dist_ema-180)*(_DUTY_MAX/180.0));
